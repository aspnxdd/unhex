...
export async function cli(): Promise<Config> {
   try {
      const actualConfig = require(join(process.cwd(), CONFIG_FILENAME)) as Config
      if (actualConfig) {
...
async function ask(): Promise<Config> {
   ...
   console.log(
      chalk.green('Configuration file created! You can edit this file manually as unhex.config.js on this directory')
   )
   return {
      extensionsAllowed,
      direction,
...
async function askExtensions(): Promise<string> {
   ...
   while (!done) {
      ...
      console.log(chalk.blue('Extensions selected: ', [...extensions].join(', ')))
   }
